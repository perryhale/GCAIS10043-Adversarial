"""
Random Forest IDS ZOO attack. Currently ZOO attack does not work on either DT or RF tree models, cause unclear.
"""
...
(test_ben_x, test_ben_y, test_ben_mask), (test_mal_x, test_mal_y, test_mal_yt, test_mal_mask) = adversarial_split(test_data[:2_000]) # truncation of test set
...
### define model
with open('randomforest_ids_sklearn.pkl', 'rb') as file:
...
conf_axis = np.linspace(CONF_MIN, CONF_MAX, num=CONF_RES)
grid_history = {
	'test_ben_acc':[], 
	'test_mal_acc':[], 
	'test_ben_adv_acc':[], 
...
for c in tqdm(conf_axis):
	
	# setup art wrapper
	art_model = SklearnClassifier(model)
	zoo_untargeted = ZooAttack(art_model, confidence=c, targeted=False, nb_parallel=ZOO_PARALLEL, batch_size=1, verbose=VERBOSE)
	zoo_targeted = ZooAttack(art_model, confidence=c, targeted=True, nb_parallel=ZOO_PARALLEL, batch_size=1, verbose=VERBOSE)
	
	# generate samples
	test_ben_adv_x = enforce_res(zoo_untargeted.generate(test_ben_x, mask=test_ben_mask), FEATURE_SCALE)
	test_mal_adv_x = enforce_res(zoo_targeted.generate(test_mal_x, test_mal_yt, mask=test_mal_mask), FEATURE_SCALE)
...
axs.set_title('randomforest_ids_sklearn')
axs.plot(conf_axis, grid_history['test_ben_adv_acc'], label='benign')
axs.plot(conf_axis, grid_history['test_mal_adv_acc'], label='malicious', c='red')
axs.plot(conf_axis, grid_history['test_mal_adv_acct'], label='malicious targeted', linestyle='dashed', c='red')
